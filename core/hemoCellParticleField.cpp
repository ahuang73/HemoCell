...
namespace hemo
{
  ...
  void HemoCellParticleField::findNonRBCParticles(
      Box3D domain, std::vector<HemoCellParticle *> &found)
  {
    found.clear();
    PLB_ASSERT(contained(domain, this->getBoundingBox()));
    for (HemoCellParticle &particle : particles)
    {
      if (this->isContainedABS(particle.sv.position, domain) && particle.sv.celltype != 0)
      {
        found.push_back(&particle);
      }
    }
...
#define inner_loop2                                                                                                                                                       \
  ...
  void HemoCellParticleField::applyBoundaryAdhesionForce(HemoCell &hemocell)
  {
    ...
    // for(int i = 0; i<CTCPLTParticles; i++){
    //   std::cout<< " particle: " << particles_per_type[1][i];
    // }
    // std::cout<<std::endl; 
    // std::cout << "boundary size: " << boundarySize << " CTC particles: "<< CTCPLTParticles<<std::endl;
    std::vector<HemoCellParticle*> nonRBCParticles;
    findNonRBCParticles(boundingBox, nonRBCParticles);

    for (int i = 0; i < nonRBCParticles.size(); i++)
    {
      hemo::Array<T, 3> cellPosition = nonRBCParticles[i]->sv.position;
      int x0 = std::floor(cellPosition[0]);
      int y0 = std::floor(cellPosition[1]);
      int z0 = std::floor(cellPosition[2]);
      for (int x = x0 - 2; x <= x0 + 2; i++)
      {
        if (x < 0 || x > this->atomicLattice->getNx() - 1)
        {
          continue;
        ...
        for (int y = y0 - 2; y <= y0 + 2; y++)
        {
          ...
          for (int z = z0 - 2; z <= z0 + 1; z++)
          {
            ...
            plb::Dot3D boundaryPosition = plb::Dot3D(x, y, z);
            int check = -1;
            bool boundaryExists = false;
            for(int j = 0; j< boundaryParticles.size(); j++){
              if(boundaryPosition == boundaryParticles[j]){
                check = j;
                boundaryExists = true;
              }
            }
            if (boundaryExists)
            {
              ...
              HemoCellParticle lParticle = *nonRBCParticles[i];
              const hemo::Array<T, 3> dv = nonRBCParticles[i]->sv.position - (boundaryPosition + this->atomicLattice->getLocation());
              const T distance = sqrt(dv[0] * dv[0] + dv[1] * dv[1] + (dv[2]) * (dv[2]));
              ...
              if (bonds.size() != 0)
              {
                ...
                {
                  if ((bonds[j][0] == __index) && (bonds[j][2] == nonRBCParticles[i]->sv.celltype) && (bonds[j][3] == nonRBCParticles[i]->sv.vertexId) && (bonds[j][4] == basecellId))
                  {
                  ...
                  else if (/*(!(bonds[j][0]==__index) && ((bonds[j][2]==ct) &&(bonds[j][3]==lParticle.sv.vertexId) && (bonds[j][4]==basecellId))) || */ (!(bonds[j][0] == __index) && ((bonds[j][2] == lParticle.sv.celltype) && (bonds[j][3] == lParticle.sv.vertexId) && (bonds[j][4] == basecellId))))
                  {
                    existed_bond = true;
                  }
                }
              ...
              { // br_cutoff
                ...
                if (lParticle.sv.celltype != 0)
                {
                  ...
                  vector<float> local_a = {__index, in, lParticle.sv.celltype, lParticle.sv.vertexId, basecellId, distance, distance_normal, boundaryPosition.x, boundaryPosition.y, boundaryPosition.z, lParticle.sv.position[0], lParticle.sv.position[1], lParticle.sv.position[2]};
                  distancesvectors.insert(distancesvectors.end(), local_a);
                }
              } // checking bond breaking finishe
            }
          }
        }
      }
      ...
      for (int i = 0; i < distancesvectors.size(); i++)
      {
        ...
        float k_on = k_on0 * exp((-1 * sigma_on * (dist2 - 5e-7) * (dist2 - 5e-7)) / (2 * kbT)); // simplification: we used distance instead of (distance - equilibrium spring length )
        P_on = (1 - exp(-1 * k_on * deltaT));
        if (dist2_n < 50e-9)
        { /*P_on =1; */
        }
