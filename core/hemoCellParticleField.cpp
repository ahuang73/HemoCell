      ...
      if (distance < 2) { \
      	if (stochastic_model){ \
                ...
                float k_off = k_off0*exp(((sigma_off-sigma_on)*dist2*dist2)/(2*kbT)); \
                 ...
                 if (distance < 0.4) {  const hemo::Array<T, 3> rfm2 = 4/(distance *0.5e-6) *0.00001*kbT *(6*pow(0.4/distance,6)-12*pow(0.4/distance,12))* (dv/distance) / df2 ; \
...
//Developed adhesion force based on stochastic model by  Nahid Rahmati
     for(auto b_particle = boundaryParticles.begin(); b_particle != boundaryParticles.end(); ++b_particle){
            ...
            if (y1 <0 || y2 > this->atomicLattice->getNy() - 1) { //cout<<"boundary:"<<__index<<endl;
                continue; }
            if (z1 <0 || z2 > this->atomicLattice->getNz() - 1) { //cout<<"boundary:"<<__index<<endl;
                continue; }
            ...
            for (int x = b_particle->x-2 ; x <= b_particle->x+2 ; x++) {
                ...
                for (int y = b_particle->y-2 ; y <= b_particle->y+2 ; y++) {
                    ...
                    for (int z = b_particle->z - 2; z <= b_particle->z + 1; z++) {
                        if (z < 0 || z > this->atomicLattice->getNz() - 2) { continue;}
                          if (y>7 && y<30 && z>7 && z<30){continue;}
                        const int &index = grid_index(x, y, z);
                                        ...
                                        float P_random_break= (float) rand()/(float)(RAND_MAX);
                                            ...
                                            if (P_off>P_random_break){
                                              ...
                                              //   cout<<"break happens"<<"; P_off: "<<P_off<<" dist2: "<< dist2 << " dist2_n: "<< dist2_n<<endl;
                                                ...
                                                j=j-1;
                                            } else{existed_bond=true;}

			           	        	} else if (  /*(!(bonds[j][0]==__index) && ((bonds[j][2]==ct) &&(bonds[j][3]==lParticle.sv.vertexId) && (bonds[j][4]==basecellId))) || */ (!(bonds[j][0]==__index) && ((bonds[j][2]==ct) &&(bonds[j][3]==lParticle.sv.vertexId) && (bonds[j][4]==basecellId)))){existed_bond=true;}
                                }
                            }  // checking bond breaking finished

                            //probabilistic adhesion function (modified by Nahid n3rahmat@uwaterloo.ca)
                            if ( distance < 3 ) { //br_cutoff
        ...
        for (int i = 0; i < distancesvectors.size(); i++) {
            for (int j=i; j<distancesvectors.size(); j++){
                if (i==j){continue;}
                if (distancesvectors[i][2]==distancesvectors[j][2] && distancesvectors[i][3]==distancesvectors[j][3] && distancesvectors[i][4]==distancesvectors[j][4]){distancesvectors.erase(distancesvectors.begin()+j); i=0;
                    j = 0;}
                           ...
                           // cout<<"x1:"<< dv[0] <<"; y1: " << dv[1] << "; z1:" <<dv[2] <<endl;
                            if ((bonds[i][2] == ct) && (bonds[i][3] == lParticle.sv.vertexId) &&
                                ...
                                const hemo::Array<T, 3> rfm =
                                    ...
                                    }else {lParticle.sv.force_adhesion = lParticle.sv.force_adhesion + 1*rfm;}
